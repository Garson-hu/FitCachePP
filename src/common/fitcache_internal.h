...
#ifndef __FitCache_INTERNAL_H__
#define __FitCache_INTERNAL_H__
...
#define FitCache_VERSION "0.0.1"

#ifdef FitCache_PRELOAD
...
#ifdef DEFINE_REAL_FUNCTIONS
    #define REAL_DEF(func, ret, args) ret (*__real_ ## func) args = NULL;
#else
    #define REAL_DEF(func, ret, args) extern ret (*__real_ ## func) args;
#endif

#define MAP_OR_FAIL(func) \
    if (!(__real_ ## func)) \
    { \
        __real_ ## func = (typeof(__real_ ## func)) dlsym(RTLD_NEXT, #func); \
        if(!(__real_ ## func)) { \
            fprintf(stderr, "fitcache failed to map symbol: %s\n", #func); \
            exit(1); \
        } \
    }
...
REAL_DECL(mmap64, void*, (void *addr, size_t length, int prot, int flags, int fd, off64_t offset))
extern void* WRAP_DECL(mmap64)(void *addr, size_t length, int prot, int flags, int fd, off64_t offset);
...
extern bool fitcache_track_file(const char* path, int flags, int fd);
extern const char * fitcache_get_path(int fd);

...
void fitcache_init_logging()
{
    ...
    if (getenv(FitCache_LOG_LEVEL))
    {
        int level = atoi(getenv(FitCache_LOG_LEVEL));
    ...
    char *param_log_dir = getenv(FitCache_LOG_DIR);
    if (param_log_dir == NULL) {
        param_log_dir = L4C_LOG_DIR;
        L4C_INFO("Using default log directory %s\n", param_log_dir);
    }
    char *param_log_prefix = getenv(FitCache_LOG_PREFIX);

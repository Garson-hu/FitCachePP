#!/bin/bash
#SBATCH -J megatron_PCF
#SBATCH -t 01:00:00
#SBATCH -N 1
#SBATCH -C nvme
#SBATCH -p batch
#SBATCH --account=gen008
#SBATCH -o /lustre/orion/gen008/proj-shared/ghu4/FitCachePP/benchmarks/results/frontier/megatron_compare/20260515_011228_megatron_compare/PCF/megatron_PCF-%j.out
...
RESULTS_DIR="/lustre/orion/gen008/proj-shared/ghu4/FitCachePP/benchmarks/results/frontier/megatron_compare/20260515_011228_megatron_compare/PCF"
cd "$RESULTS_DIR"

# FitCache env (applies to both modes; PCF just doesn't LD_PRELOAD)
...
# Spawn the FitCache server (PCF mode also spawns it but skips LD_PRELOAD
# so the python process bypasses it entirely)
...
# torchrun expects a master addr/port. Single GPU still needs distributed init.
...
# Megatron-LM bypass-apex/TE flags for ROCm 6 without Transformer Engine.
# These are the minimum set v0.11.0 needs to run on bare PyTorch+ROCm.
MEGATRON_ARGS=(
    --num-layers 12
    --hidden-size 768
...
END=$SECONDS
ELAPSED=$((END - START))
echo "=== mode=PCF done $(date) wall=${ELAPSED}s ==="

kill -TERM $SERVER_PID 2>/dev/null || true
sleep 2

# Engagement signals — only meaningful for FCP
